...
"""
When we place a ship it should stay within board boundary
"""

def test_ships_stay_within_boundary_less_than_0():
    game = Game()
    game.place_ship(length=3, orientation="vertical", row=0, col=0)
    assert game.ship_at(1, 1)
    assert game.ship_at(2, 1)
    assert game.ship_at(3, 1)

def test_ships_stay_within_boundary_horizontal_greater_than_10():
    game = Game()
    game.place_ship(length=3, orientation="horizontal", row=5, col=9)
    assert game.ship_at(5, 8)
    assert game.ship_at(5, 9)
    assert game.ship_at(5, 10)

def test_ships_stay_within_boundary_vertical_greater_than_10():
    game = Game()
    game.place_ship(length=3, orientation="vertical", row=9, col=5)
    assert game.ship_at(8, 5)
    assert game.ship_at(9, 5)
    assert game.ship_at(10, 5)