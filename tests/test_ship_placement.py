...
"""
Ship constrained to the board horizontal greater than 10
"""

def test_ship_cannot_leave_board_horizontal_greater_than_10():
    Ship_placement = ShipPlacement(
        length=5, orientation='horizontal', row=9, col=9)
    assert str(
        Ship_placement) == "ShipPlacement(length=5, orientation=horizontal, row=9, col=6)"
    
"""
Ship constrained to the board vertical greater than 10
"""
def test_ship_cannot_leave_board_vertical_greater_than_10():
    Ship_placement = ShipPlacement(
        length=5, orientation='vertical', row=8, col=9)
    assert str(
        Ship_placement) == "ShipPlacement(length=5, orientation=vertical, row=6, col=9)"
    

"""
Ship constrained to the board horizontal less than 1
"""
def test_ship_cannot_leave_board_horizontal_less_than_1():
    Ship_placement = ShipPlacement(
        length=5, orientation='horizontal', row=5, col=0)
    assert str(
        Ship_placement) == "ShipPlacement(length=5, orientation=horizontal, row=5, col=1)"
    
"""
Ship constrained to the board vertical less than 1
"""
def test_ship_cannot_leave_board_vertical_less_than_1():
    Ship_placement = ShipPlacement(
        length=5, orientation='vertical', row=0, col=5)
    assert str(
        Ship_placement) == "ShipPlacement(length=5, orientation=vertical, row=1, col=5)"
